...
def generate_single_image(prompt_data: Dict, api_token: str, model_params: Dict) -> Dict:
    """Generate a single image using Flux 2"""
    try:
        os.environ["REPLICATE_API_TOKEN"] = api_token
        
        # Flux 2 model on Replicate
        output = replicate.run(
            "black-forest-labs/flux-1.1-pro",  # Using Flux 1.1 Pro (latest available)
            input={
                "prompt": prompt_data['prompt'],
                "aspect_ratio": model_params.get('aspect_ratio', '1:1'),
                "output_format": model_params.get('output_format', 'png'),
                "output_quality": model_params.get('output_quality', 100),
                "safety_tolerance": model_params.get('safety_tolerance', 2),
                "prompt_upsampling": model_params.get('prompt_upsampling', True)
            }
        )
...
with st.sidebar:
    ...
    try:
        ...
        if not api_token:
            st.info("💡 For deployment: Add REPLICATE_API_TOKEN to Streamlit secrets")
    ...
    prompt_upsampling = st.checkbox("Enable Prompt Upsampling", value=True, 
                                     help="Enhances prompts for better results")
...
with tab1:
    ...
    with col2:
        ...
        estimated_cost = num_images * 0.01  # Approximate Flux cost
        estimated_time = num_images / max_workers * 10  # ~10 seconds per image
